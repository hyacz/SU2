    ...
    if (TimeIter == 0){
      ...
      for (iMesh=0; iMesh<=config[val_iZone]->GetnMGLevels();iMesh++) {
        for(iPoint=0; iPoint<geometry[val_iZone][val_iInst][iMesh]->GetnPoint();iPoint++) {
          ...
          if (grid_IsMoving) {
            geometry[val_iZone][val_iInst][iMesh]->node[iPoint]->SetCoord(geometry[val_iZone][val_iInst][iMesh]->nodes->GetCoord_n(iPoint));
          }
          if (turbulent) {
      ...
      if (dual_time_1st){
        ...
        for (iMesh=0; iMesh<=config[val_iZone]->GetnMGLevels();iMesh++) {
          for(iPoint=0; iPoint<geometry[val_iZone][val_iInst][iMesh]->GetnPoint();iPoint++) {
            ...
            if (grid_IsMoving) {
              geometry[val_iZone][val_iInst][iMesh]->nodes->SetCoord_n(iPoint, geometry[val_iZone][val_iInst][iMesh]->node[iPoint]->GetCoord_Old());
            }
            if (turbulent) {
      ...
      if (dual_time_2nd){
        ...
        for (iMesh=0; iMesh<=config[val_iZone]->GetnMGLevels();iMesh++) {
          for(iPoint=0; iPoint<geometry[val_iZone][val_iInst][iMesh]->GetnPoint();iPoint++) {
            ...
            if (grid_IsMoving) {
              geometry[val_iZone][val_iInst][iMesh]->nodes->SetCoord_n1(iPoint, geometry[val_iZone][val_iInst][iMesh]->node[iPoint]->GetCoord_Old());
            }
            if (turbulent) {
